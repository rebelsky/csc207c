...
In this example, `Student` and `Professor` have `age` and `gender` fields and `answer` and `askQuestionOf` methods, even though neither explicitly declares them.
...
Of course, when you write a new class, you want to do more than create new fields and new methods (and reuse old fields and new methods); there are also times that you want to change the behavior of the old methods.  The term for such a change is *overriding*.

To override a method, you simply write the new version of the method.  You also add a `@Override` tag to let Java know that you are intentionally overriding a method.  (That tag is optional, but good practice.)
...
That is, even though `p` is declared as a `Person`, we get the appropriate method from the object to which it refers.
