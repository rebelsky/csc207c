---
title: "Eboard 09 (Section 1): Dictionaries and Associative Arrays"
number: 9
section: eboards
held: 2024-09-26
link: true
...
* Schedule switch! No readings for Tuesday, no lab for today.
...
`get` - iterate through the array, looking at the key of each pair
...
`set` - (v2) Iterate through the array, looking at the key of each pair.
...
* Note: If `set` is just "add a new pair to the end", we should search 
  backwards from the end, rather than forwards from the front.

`hasKey` - Iterate throught the array. If we hit the the end of the
array or an empty spot, return false.  If we hit a matching key along
the way, return true.

`remove` (v1) - Iterate through the array to find the space. Make that point
to an empty key/value pair.

`remove` (v2) - Iterate through the array to find the space. Put null in
the key and value parts of that pair.

`remove` (v3) - Iterate through the array to find the space, Put null
in that space.

`remove` (v4) - Iterate through the array to find the space, swap the
last element into that space and put a null at the end.

`remove` (v5) - Iterate through the array to find the space, shove everything
...
`remove` - As many elements as there are in the array, plus the additional
effort of removing
...
* `remove` v1, v2, and v3 are likely to add complications because all of
...
* `set`, `get`, and `hasKey` all have to iterate through the array, so
  we should have a helper method. `int find(K key)`, which returns the
